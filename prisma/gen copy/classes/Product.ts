...
export class _Product extends PrismaClass {
	...
	toJSON(ids: boolean = false) {
		return {
			...
			productState: ids ? undefined : this.productState,
			productVisibilty: ids ? undefined : this.productVisibilty,
			sub_orders: ids ? undefined : this.sub_orders,
			product_categories: ids ? undefined : this.product_categories,
			gallery: ids ? undefined : this.gallery,
		}
	}

	private nonRelationsToJSON() {
		return {
			id: this.id!,
			vendor_id: this.vendor_id!,
			state: this.state!,
	...
	async *saveToTransaction(
		...
		this._id = (await this.prisma.upsert({
			where: { id: this.id },
			create: { ...this.nonRelationsToJSON(), id: undefined },
			update: { ...this.nonRelationsToJSON(), id: undefined },
			select: { id: true }
		})).id
